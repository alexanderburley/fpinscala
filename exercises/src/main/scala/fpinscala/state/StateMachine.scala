package fpinscala.statemachine
import State._
...
object State {
  ...
  def modify[S](f: S => S): State[S, Unit] =
    this.get.flatMap(s => this.set(f(s)))

  def modifyAnswer[S](f: S => S): State[S, Unit] =
    for {
...
object Candy {
  ...
  // for(_ <- c1; y <- c2) yield {y.coins, y.candies}
  //(x => c2.flatMap(y => c3.map(z => {...})))
  ...
  def simulateMachine(inputs: List[Input]): State[Machine, (Int, Int)] = {
    // Map the inputs into a list of modifications based on update
    val instructions = inputs.map(i => {
      val modifier: (Machine => Machine) => State[Machine, Unit] =
        modify[Machine] _
      val composedUpdate: Input => State[Machine, Unit] =
        modifier compose update
      composedUpdate(i)
    })
    val combined = sequence(instructions)
    val res: State[Machine, Machine] = combined.flatMap(s => get)
    res.map(x => (x.coins, x.candies))
  }
